...
for(my $i=0;$i<@reads;$i++){
	...
	for(my $j=0;$j<@reads;$j++){
		...
		next if($mm < 2);
		$nn=(scalar(@cs)+1)/3;
		my $output=align->new(\@cs, \@pr, \%dd1, \%dd2, \%ann_len,\%ann_from, \%ann_to, $min_fragment_length,$cutoff_similarity);
		my $rev=0;
		if($output->{max_score} < 2 and $output->{max_score} != -1){ # reverse the partner fragments
			@pr=&revfrag(@pr);
			$output=align->new(\@cs, \@pr, \%dd1, \%dd2, \%ann_len,\%ann_from, \%ann_to, $min_fragment_length, $cutoff_similarity);
			$rev=1;
		}
